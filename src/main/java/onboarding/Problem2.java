...
public class Problem2 {
    ...
    public static String removeSeqChar(String cryptogram) {
        ...
        StringBuilder cryptogramBuilder = new StringBuilder(cryptogram); // String 객체는 불변 따라서 StringBuilder 객체로 변환시켜서 사용함
        StringBuilder stringBuilder = null; // 임시 공간
        int dup_char_end_index = 0;

        for(int i = 0; i < cryptogramBuilder.length() - 1; i++) {
            dup_char_end_index = check_seq_char(cryptogramBuilder.toString(), i, cryptogramBuilder.charAt(i));

            if (i != dup_char_end_index) { // 연속된 문자가 아니라면
    ...
    public static int check_seq_char(String cryptogram, int i, char c) {
